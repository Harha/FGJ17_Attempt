...
void PlayState::render(Display * const display)
{
	...
	if (m_game->getRunState() == GRS_RUNNING_DBG)
	{
		...
		display->drawText(font, "Entities: " + std::to_string(m_level.getEntityVector().size()), textcolor, vec2(2, 66));
		display->drawText(font, "Player s: " + m_level.getPlayer()->getPosition().toString(), textcolor, vec2(2, 82));
		display->drawText(font, "Player v: " + m_level.getPlayer()->getVelocity().toString(), textcolor, vec2(2, 98));
		display->drawText(font, "Player MoveDirX: " + std::to_string(m_level.getPlayer()->getMoveDirX()), textcolor, vec2(2, 114));
		display->drawText(font, "Player MoveDirY: " + std::to_string(m_level.getPlayer()->getMoveDirY()), textcolor, vec2(2, 130));
	}
}
